...
export class GraphUsuarios implements UsuariosPort {
  constructor(private readonly tokens: EntraTokenProvider, private readonly cfg: { dominio: string; excluir: string[] } = { dominio: "", excluir: [] }, private readonly fetchImpl: typeof fetch = fetch) {}

  async listar(): Promise<UsuarioDir[]> {
    const token = await this.tokens.getToken(GRAPH_SCOPE);
    const url = `${GRAPH}/users?$select=id,displayName,mail,userPrincipalName,employeeId,userType&$filter=accountEnabled eq true&$top=999`;
