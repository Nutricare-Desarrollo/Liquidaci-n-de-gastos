...
export const api = {
  ...
  ingestaCorreo: () => req<{ procesados: number; ingestadas?: number; ignoradas?: number }>("/jobs/ingesta-correo", { method: "POST" }),
  ...
  duplicarGasto: (id: string) => req<{ ok: boolean; error?: string; gastoId?: string }>(`/gastos/${id}/duplicar`, { method: "POST" }),
  crearGastoManual: (liqId: string, facturaId: string, categoriaId: string, informacionAdicional?: string) => req<{ ok: boolean }>(`/liquidaciones/${liqId}/gastos`, { method: "POST", body: JSON.stringify({ facturaId, categoriaId, informacionAdicional }) }),
  ...
  eliminarFactura: (id: string, force = false) => req<{ ok: boolean; error?: string; gastosEliminados?: number }>(`/facturas/${id}${force ? "?force=1" : ""}`, { method: "DELETE" }),
  capturas: () => req<Record<string, unknown>[]>("/capturas"),
  eliminarCaptura: (id: string) => req<{ ok: boolean; error?: string }>(`/capturas/${id}`, { method: "DELETE" }),
  ...
  getConfigRoles: () => req<Record<string, string>>("/config/roles"),
  guardarConfigRoles: (roles: Record<string, string>) => req<{ ok: boolean }>("/config/roles", { method: "PUT", body: JSON.stringify({ roles }) }),
